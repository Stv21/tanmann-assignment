...
import { RiBodyScanFill, RiMenuUnfoldFill, RiMenuFoldLine } from "react-icons/ri";
...
const Sliderbar = () => {
  ...
  const menuItems = [
    { path: `/users/home/${userId}`, name: "Home", icon: <FaHome /> },
    { path: `/users/profile/${userId}`, name: "Profile", icon: <CgProfile /> },
    { path: `/users/bodymeasurements/${userId}`, name: "Body Measurements", icon: <RiBodyScanFill /> }, /*icon not found on ri*/
    { path: `/users/accountsettings/${userId}`, name: "Account Settings", icon: <IoSettingsOutline /> },
    // { path: `/users/orders/${userId}`, name: "Orders", icon: <RxCodesandboxLogo /> }, /*icon not found on ri*/
    // { path: `/users/tailorrecommendation/${userId}`, name: "Tailor Recommendation", icon: <LiaTapeSolid /> }, /*icon not found*/
    { path: `/users/sellerRequest/${userId}`, name: "Request Seller", icon: <IoWomanSharp /> },
    // { path: `/users/savedtailors/${userId}`, name: "Saved Tailors", icon: <FaCheck /> },
    { path: `/users/support/${userId}`, name: "Support", icon: <BiSupport /> },
    // { path: `/users/logout/${userId}`, name: "Logout", icon: <FaPowerOff /> }
  ];
  ...
  return (
    <nav className={`h-full border-[2px] border-solid border-transparent rounded-md ease-in-out duration-700 pb-16 bg-white z-50 ${isOpen ? "sm:w-72 w-[235px] absolute md:static md:bg-transparent" : "w-12"} overflow-hidden mr-3`}>
      <div className="flex items-center h-20 w-full border-b-2 border-solid border-[#D3D3D3]">
        <img src={Logo} className={`ease-in-out duration-700 ${isOpen ? "w-36 h-20 " : "w-0 h-full"}`} alt="Logo" />
      ...
      <div className='h-full overflow-y-scroll scrollbar-hide mt-3'>
        {menuItems.map((item, index) => (
          <NavLink
            ...
            key={index}
            className={({ isActive }) =>
              `flex items-center whitespace-nowrap py-3 px-3 gap-4 mb-2 no-underline transition-all duration-500 ${isActive ? "bg-[#F8444F] text-white rounded-md" : "text-[#135049]"}  hover:bg-[#F8444F] hover:text-white hover:rounded-md`
            }
          >
